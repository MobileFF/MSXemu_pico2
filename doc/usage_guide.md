...
実行時メニュー（GUI+F7）の `Save State` / `Load State` から、CPU・VDP・RAM・VRAMの状態を丸ごとSDカードに保存・復元できます。セーブファイルは約80KB。カートリッジごとにROM本体の隣（例: `Foo.ROM` → `Foo.0.sav`）へ、最大10世代までローテーションしながら保存されます（`Load State` から過去の世代を選んで読み込み可能）。
