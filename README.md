...
- **実行時メニュー**: USBキーボードの GUI+F7 でオンスクリーンメニューを起動。カートリッジ交換、セーブ/ロード、リセット、音量/音質調整、表示設定(LCD/HDMI切替含む)をゲームを止めずに操作可能。
...
- **メガロム(バンク切替対応の大容量カートリッジ)をHDMI出力と併用すると、動作がやや遅くなることがあります**(内蔵フラッシュキャッシュ読み込みの影響)。実行時メニューの「Display Settings」で Frame Skip (`hdmi_frame_skip`) を調整してください。
